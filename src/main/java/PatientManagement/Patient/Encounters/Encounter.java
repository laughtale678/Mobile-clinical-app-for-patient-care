...
public class Encounter {
    ...
    public Encounter(Patient p, String cc, Event ev, EncounterHistory eh) { // event is the date when the check was made
        ...
        // add this encounter to the event's encounters list
    ...
    public void newReferral(String type) {
        Clinic c = patient.getClinic();
        for(ThirdPartyService tps : c.getThirdPartyServiceCatalogs().getThirdPartyServices()) {
            if(tps.getSite() == event.getSite() && tps.getType().equals(type)){
                referral = new Referral(tps);
                break;
            }
        }
    }
